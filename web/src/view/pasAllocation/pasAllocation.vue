    ...
    <el-table-column label="结束日期" prop="EndingDate" width="120"></el-table-column> 
      ...
      <el-table-column label="按钮组">
        <template slot-scope="scope">
          <el-button class="table-button" @click="updatePasAllocation(scope.row)" size="small" type="primary" icon="el-icon-edit">变更</el-button>
          <el-popover placement="top" width="160" v-model="scope.row.visible">
            <p>确定要删除吗？</p>
            <div style="text-align: right; margin: 0">
              <el-button size="mini" type="text" @click="scope.row.visible = false">取消</el-button>
              <el-button type="primary" size="mini" @click="deletePasAllocation(scope.row)">确定</el-button>
            </div>
...
import {
    createPasAllocation,
    deletePasAllocation,
    deletePasAllocationByIds,
    updatePasAllocation,
    findPasAllocation,
    getPasAllocationList
...
export default {
  ...
  methods: {
      ...
      async onDelete() {
        ...
        const res = await deletePasAllocationByIds({ ids })
        if (res.code == 0) {
          this.$message({
            type: 'success',
            message: '删除成功'
    ...
    async updatePasAllocation(row) {
      const res = await findPasAllocation({ ID: row.ID });
      this.type = "update";
      if (res.code == 0) {
    ...
    async deletePasAllocation(row) {
      this.visible = false;
      const res = await deletePasAllocation({ ID: row.ID });
      if (res.code == 0) {
        this.$message({
          type: "success",
          message: "删除成功"
