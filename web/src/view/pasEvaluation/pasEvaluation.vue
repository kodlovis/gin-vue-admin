    ...
    <el-table-column label="Status字段" prop="Status" width="120"></el-table-column> 
      ...
      <el-table-column label="按钮组">
        <template slot-scope="scope">
          <el-button class="table-button" @click="updatePasEvaluation(scope.row)" size="small" type="primary" icon="el-icon-edit">变更</el-button>
          <el-popover placement="top" width="160" v-model="scope.row.visible">
            <p>确定要删除吗？</p>
            <div style="text-align: right; margin: 0">
              <el-button size="mini" type="text" @click="scope.row.visible = false">取消</el-button>
              <el-button type="primary" size="mini" @click="deletePasEvaluation(scope.row)">确定</el-button>
            </div>
...
import {
    createPasEvaluation,
    deletePasEvaluation,
    deletePasEvaluationByIds,
    updatePasEvaluation,
    findPasEvaluation,
    getPasEvaluationList
...
export default {
  ...
  methods: {
      ...
      async onDelete() {
        ...
        const res = await deletePasEvaluationByIds({ ids })
        if (res.code == 0) {
          this.$message({
            type: 'success',
            message: '删除成功'
    ...
    async updatePasEvaluation(row) {
      const res = await findPasEvaluation({ ID: row.ID });
      this.type = "update";
      if (res.code == 0) {
    ...
    async deletePasEvaluation(row) {
      this.visible = false;
      const res = await deletePasEvaluation({ ID: row.ID });
      if (res.code == 0) {
        this.$message({
          type: "success",
          message: "删除成功"
